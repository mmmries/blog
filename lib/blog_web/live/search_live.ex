defmodule BlogWeb.SearchLive do
  ...
  def modern_post_link(assigns) do
    ~H"""
    <a class="block px-4 py-3 text-decoration-none border-b border-gray-100 last:border-b-0 transition-colors duration-150 ease-in-out hover:bg-gray-50" href={@post.path}>
      <div class="text-sm font-medium text-gray-900 leading-tight mb-0.5">{@post.title}</div>
      <div class="text-xs text-gray-600 leading-tight">{Calendar.strftime(@post.date, "%B %d, %Y")}</div>
    </a>
    """
  end
